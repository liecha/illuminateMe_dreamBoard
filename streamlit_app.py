...
def create_new_day(this_date, df_new):
    df_db = pd.read_csv('data/updated-database-developer.csv')
    df_db = df_db[df_db.date != this_date]
    df_basal_energy = basal_energy(this_date)       
    df_new_post = df_new[df_new['date'] == this_date]   
    ...
    df_energy_new.to_csv('data/updated-database-developer.csv', index=False)
...
def build_dashboard(key_selector):
    bmr = 1360 #bmr(50, 170, 42)
    date_now, time_now =  date_time_now()   
    selected_date = st.date_input("Select a date", date_now, key=key_selector)
    ...
    if len(df_energy_date) > 0:
        ...
        this_date = df_activity_irl['date'].iloc[0]
        button_pressed = st.button("Save changes")
        if button_pressed:
            df_new = edited_df[edited_df.delete == False]
            df_new = df_new.drop(['delete'], axis=1)
            create_new_day(this_date, df_new)
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    ...
    "🍛 Meal Registration",
    "🗄️ Add food item to database",
    "🗄️ Add recipie to database",
    ])
with tab1:
    build_dashboard('head_key_1')
...
with tab3:
    ...
    with col[0]:  
        ...
        options = st.multiselect(
            "Select food items to your meal",
            food_list,
            key='create_meal'
        )
